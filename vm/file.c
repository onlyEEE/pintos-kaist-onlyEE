...
file_backed_swap_in(struct page *page, void *kva)
{
	...
	memset(kva + file_info->read_bytes, 0, PGSIZE - file_info->read_bytes);
...
file_backed_swap_out(struct page *page)
{
	...
	if (is_dirty)
	{
		if (page->is_writable)
		{
			int check = file_write_at(file_info->file, page->va, file_info->read_bytes, file_info->ofs);
			if (check != file_info->read_bytes)
				return false;
