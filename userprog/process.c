...
{
	struct thread *curr = thread_current(); // child process
	/* TODO: Your code goes here.
...
void argument_stack(char **parse, int count, struct intr_frame *if_)
{
	...
	for (i = count - 1; i > -1; i--)
	{
		align_size = strlen(parse[i]) + 1;
		if_->rsp = if_->rsp - align_size;
		memcpy(if_->rsp, parse[i], align_size);
...
bool lazy_load_segment(struct page *page, void *aux)
{
	...
	if (page->frame->write_protected > 1)
		return true;
	...
	// printf("file_info->zero_bytes %d\n", file_info->zero_bytes);
	...
	// if (file_info->ofs < 0) return false;
